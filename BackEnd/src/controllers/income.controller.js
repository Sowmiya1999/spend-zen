import {
  ADD_INCOME_FAILED_ERROR_MESSAGE,
  DOWNLOAD_INCOME_EXCEL_FAILED_ERROR_MESSAGE,
  DOWNLOAD_INCOME_EXCEL_SUCCESS_MESSAGE,
  GET_INCOME_FAILED_ERROR_MESSAGE,
  INCOME_ADDED_SUCCESS_MESSAGE,
  INCOME_DELETED_SUCCESS_MESSAGE,
  INCOME_DELETION_FAILED_ERROR_MESSAGE,
  INCOME_FETCHED_SUCCESS_MESSAGE,
...
class IncomeController {
  ...
  addIncome = async (req, res) => {
    try {
      ...
      let incomeData = await this.incomeService.addIncomeService(
        req.userId,
        source,
        icon,
        date,
      ...
      return res
        .status(200)
        .json({ message: INCOME_ADDED_SUCCESS_MESSAGE, data: incomeData });
    } catch (err) {
      console.log(`IncomeController.addIncome produced error: ${err}`);
      return res
        .status(500)
        .json({ message: ADD_INCOME_FAILED_ERROR_MESSAGE, error: err });
    }
  };
  ...
  getAllIncome = async (req, res) => {
    try {
      ...
      const incomeData = await this.incomeService.getAllIncomeService(
        req.userId
      );
      return res
        .status(200)
        .json({ message: INCOME_FETCHED_SUCCESS_MESSAGE, data: incomeData });
    } catch (err) {
      console.log(`IncomeController.getAllIncome produced error: ${err}`);
      return res
        .status(500)
        .json({ message: GET_INCOME_FAILED_ERROR_MESSAGE, error: err });
    }
  };
  ...
  deleteIncome = async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: INCOME_DELETION_FAILED_ERROR_MESSAGE, error: err });
    }
  };
  ...
  downloadIncomeExcel = async (req, res) => {
    try {
      ...
      console.log(
        `IncomeController.downloadIncomeExcel produced error: ${err}`
      );
      return res
        .status(500)
        .json({
          message: DOWNLOAD_INCOME_EXCEL_FAILED_ERROR_MESSAGE,
          error: err,
        });
    }
  };
